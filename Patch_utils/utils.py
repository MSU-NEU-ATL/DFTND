...
def dataloader(train_size, test_size, adv_size, data_dir, batch_size, num_workers, total_num=50000):
    ...
    adv_index = index[(train_size + test_size): (train_size + test_size + adv_size)]
    ...
    adv_dataset = torchvision.datasets.ImageFolder(root=data_dir, transform=adv_transforms)
    ...
    adv_loader = DataLoader(dataset=adv_dataset, batch_size=batch_size, sampler=SubsetRandomSampler(adv_index),
                            num_workers=num_workers, pin_memory=True, shuffle=False)
...
def test(model, dataloader):
    ...
    with torch.no_grad():
        ...
        # print('outputs:{}'.format(outputs.data.size()))
...
def log_generation(log_dir):
    ...
    with open(log_dir, 'r', newline='') as f:
        reader = csv.reader(f)
        flag = 0
        for i in reader:
            if flag == 0:
    ...
    plt.legend()
    # plt.savefig(
    #     "E:/Academy/LAB/Diagnosis for Adversaries/Adversarial_Patch_Attack-master/training_pictures/patch_attack_success_rate.jpg")
    plt.savefig("/home/liyize/Codes/AdvTool/result/Patch/training_results/patch_attack_success_rate.jpg")
    plt.close(0)
