...
from Lpnorm_attacks import PGD
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task', type=str, default='C', choices=['C', 'D', 'F'], help='Classification, Detection, '
                                                                                       'Face Recognition')
    parser.add_argument(
        '--attack_type', type=str, default='PGD', choices=['FGSM', 'CW', 'PGD', 'Patch']
    )
    parser.add_argument(
    ...
    parser.add_argument('--result_dir', type=str, default='result/PGD', help="dir of the result")
    ...
    path = os.path.join(args.result_dir, 'PGD_log.csv')
    ...
    PGD = PGD.PGD(model, eps=16 / 255, alpha=5 / 255, steps=100)
    ...
    for images, labels in img_loader:
        ...
        pred_class = idx2label[int(pred_label)]
        ...
        if args.attack_goal == 'T':
            ...
            target_map_function = lambda images, labels: labels.fill_(args.attack_target)
            PGD.set_attack_mode("targeted", target_map_function=target_map_function)
        ...
        im = adv_images.squeeze(0).data.float().cpu().detach().numpy()
        # im = np.round((np.transpose(im, (1, 2, 0)) * STD + MEAN) * 255.0).astype(np.uint8)
        im = (np.transpose(im, (1, 2, 0)) * 255.0).astype(np.uint8)
        ...
        adv_images1 = preprocess(im).float()
        adv_images1 = adv_images1.unsqueeze(0)
        ...
        print('Adv%:', 100 * round(pred1_prob, 6))
        ...
        # STD = np.array([0.229, 0.224, 0.225])
        #
        # im = adv_images1.squeeze(0).data.cpu().detach().numpy()
        ...
        im.save('/home/liyize/Codes/AdvTool/result/PGD/{}_{}_{}_{}_{}_{}_{}.jpg'.format(args.task,
                                                                                        args.attack_type,
                                                                                        args.attack_goal,
                                                                                        args.model_type,
                                                                                        args.strategy, i,
        ...
        with open(path, 'a', newline='') as f:
            ...
            writer.writerow(
                [i, pred_class, round(pred_prob * 100, 3), pred1_class, round(pred2_prob * 100, 3),
                 pred_class, round(pred3_prob * 100, 3), pred1_class, round(pred1_prob * 100, 3), status])

        i = i + 1

    # print('Robust accuracy: %.2f %%' % (100 * float(correct) / total))
    print('Generate Success rate: %.2f %%' % (100 * float(counts) / total))
