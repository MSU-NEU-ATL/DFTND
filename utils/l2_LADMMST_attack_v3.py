...
CONFIDENCE = 0.1  # how strong the adversarial example should be
    ...
    def attack_batch(self, imgs, labs):
        ...
        for q in range(Q):
            ...
            for p in range(P):
                ...
                #print(p,index_)
                index[tmpidx] = index_
                #index2[tmpidx] = index2_
                
                tmpidx += 1

        index = np.tile(index, (batch_size,1,1))
        #index2 = np.tile(index2, (batch_size,1,1))
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS + outer_step * 1000 ):
                if iteration % 200 == 0:
                    print(iteration, o_bestl2, np.count_nonzero((np.array(o_bestattack) - imgs).reshape([batch_size,-1]), axis = 1))
                ...
                #for b in range(batch_size):
                    ...
                    #--------------------------- for j in range(index.shape[1]):
                        #-------------------------------------- # print(p,index)
                        #---------------------- y0D = np.take(y0[b], index[b,j])
                        #---------------------------- y0Dn = np.linalg.norm(y0D)
                        #------------ tmpy = 0 if y0Dn == 0 else 1 - tmpc / y0Dn
                        #------------------------------------------ if tmpy > 0:
                            #-------------- np.put(y[b], index[b,j], tmpy * y0D)
                            #------------ np.put(yt[b], index2[b,j], tmpy * y0D)
                        #------------------------------------------------- else:
                            #----------------------- np.put(y[b], index[b,j], 0)
                            #--------------------- np.put(yt[b], index2[b,j], 0)
                        
       
                list(map(findIndx, range(batch_size)))
                #print(time.time()-timestart) [0.57831746, 0.5190017, 0.1759373, 0.7652166, 0.34952274, 4.9221044, 0.33907297, 1.36878, 1.015729, 0.30631745, 1.0236182, 0.022604192, 0.2451056, 0.14963232, 1.1933888, 0.43752143, 1.0042298, 0.20931265, 0.7649485, 0.5501644]
